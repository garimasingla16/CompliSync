...
# Classify Policy Sentences
def sentence_sbert_classify_policy(policy_sentences, classifier, threshold=0.8):
    results = []
    unique_labels = set()

    for sentence in policy_sentences:
        if len(sentence.split()) > 11:  # Ignore short sentences
            embedding = sentence_sbert_model.encode(sentence, convert_to_tensor=True).to(device)
...
if __name__ == "__main__":
    ...
    # Find Best Examples for Each Principle
    best_examples = {}
    for sentence, labels in policy_results:
        for label, score in labels:
            if label not in best_examples or score > best_examples[label][1]:
    ...
    # Debugging - Print Compliance Results
    print("Final compliance results:")
    for principle, details in sbert_sentences_policy_results.items():
        print(f"{principle}: {'Compliant' if details.get('compliant', False) else 'Non-compliant'}")

    # Debugging - Print Scores
    for principle, example in best_examples.items():
        print(f"Principle: {principle}, Score: {example[1]}")
